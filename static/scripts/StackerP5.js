...
p.setup = function () {
  let canvas = p.createCanvas(462, 660);
  ...
  rowsPerLevel = 1;
  canvasWidth = 7;
  canvasHeight = canvasWidth * p.height / p.width;
...
//   }
//   try {
//     while ((line = reader.readp.line()) != null) {
...
p.draw = function () {
  ...
  drawBackground();
  // strokeWeight(10);
  p.line(0, tileSize * rowsPerLevel, p.width, tileSize * rowsPerLevel);
  // p.noStroke();
...
const drawBackground = () => {
  ...
  for (let i = 0; i < canvasWidth; i++) {
    for (let j = startHeight; j < canvasHeight; j++) {
      ...
      //   p.fill(120, 0, 120);
      // }
      p.rect(margin + i * (size + margin * 2), margin + j * (size + margin * 2), size, size);
    }
  }
  p.translate(0, -shift);
};
...
class Block {
  ...
  draw () {
    p.push();
    p.translate(this.x + size / 2, this.y + size / 2);
